...
from ai_coder_agent import (
    Plan, Analysis, GeneratePlan, GenerateCode, AnalyzeResult,
    execute_code, extract_code, AICoderAgent
)
import dspy
...
class TestPydanticModels:
    ...
    def test_plan_empty_tasks(self):
        """Test Plan with empty tasks list."""
        plan = Plan(description="Empty plan", tasks=[])
    ...
    def test_analysis_validation_error(self):
        """Test Analysis model validation with invalid data."""
        with pytest.raises(ValidationError):
            Analysis(description="Invalid", completed="not_a_boolean")
...
class TestAICoderAgent:
    ...
    def test_agent_run_success_flow(self, mock_print, mock_execute, agent):
        ...
        mock_execute.return_value = "--- Execution Log ---\nSTDOUT:\ntest\n--------------------"
